...
class TestInitScriptsDirSkipConditions:
    ...
    def test_skips_when_pytest_detected(self, tmp_path: Path, monkeypatch):
        ...
        prompts_dir = tmp_path / "usr" / "share" / "aish" / "prompts"
        prompts_dir.mkdir(parents=True)
        (prompts_dir / "test.aish").write_text("test content")
        ...
        # Since pytest is running, scripts dir should be empty or not have the test file
        scripts_dir = config.config_dir / "scripts"
...
class TestInitScriptsDirFileCopying:
    ...
    def test_copies_aish_files_from_prompts_dir(self, tmp_path: Path):
        """Should copy .aish files from system prompts to user scripts dir."""
        # Create mock prompts directory
        prompts_dir = tmp_path / "prompts"
        prompts_dir.mkdir()
        (prompts_dir / "script1.aish").write_text("script 1")
        (prompts_dir / "script2.aish").write_text("script 2")
        (prompts_dir / "readme.txt").write_text("should be ignored")
        ...
        removed = _remove_pytest_from_modules()
        try:
            # Patch the hardcoded system paths to use our test prompts directory
            original_path = Path

            def mock_path_constructor(arg):
                if arg == "/usr/share/aish/prompts":
                    return prompts_dir
            ...
            with patch("aish.config.Path") as mock_path:
                ...
                mock_path.return_value.iterdir = lambda: [
                    prompts_dir / "script1.aish",
                    prompts_dir / "script2.aish",
                    prompts_dir / "readme.txt",
                ]
                ...
                def side_effect_path(arg=""):
                    if arg == "/usr/share/aish/prompts":
                        ...
                        return mock_p
                    elif arg == "/usr/local/share/aish/prompts":
                        mock_p = MagicMock(spec=Path)
    ...
    def test_does_not_overwrite_existing_scripts(self, tmp_path: Path):
        ...
        # Existing script should not be changed (no system prompts dir to copy from)
    ...
    def test_only_copies_aish_extension(self, tmp_path: Path):
        ...
        prompts_dir = tmp_path / "prompts"
        prompts_dir.mkdir()
        (prompts_dir / "valid.aish").write_text("valid")
        (prompts_dir / "invalid.txt").write_text("invalid")
        (prompts_dir / "invalid.md").write_text("invalid")
        (prompts_dir / "invalid").write_text("invalid")
        ...
        # Since there's no real system prompts dir, we verify the logic through
...
class TestInitScriptsDirWithRealFiles:
    ...
    def test_copies_files_from_real_prompts_dir(self, tmp_path: Path):
        """Test actual file copying with a real prompts directory structure."""
        # Set up a real prompts directory
        prompts_dir = tmp_path / "system_prompts"
        prompts_dir.mkdir()
        (prompts_dir / "script1.aish").write_text("# Script 1\ncontent")
        (prompts_dir / "script2.aish").write_text("# Script 2\ncontent")
        (prompts_dir / "readme.txt").write_text("readme")
        ...
        try:
            ...
            with patch("aish.config.Path") as mock_path_class:
                def path_side_effect(arg=""):
                    if arg == "/usr/share/aish/prompts":
                        mock_prompts = MagicMock(spec=Path)
                        mock_prompts.is_dir.return_value = True
                        mock_prompts.iterdir.return_value = [
                            prompts_dir / "script1.aish",
                            prompts_dir / "script2.aish",
                            prompts_dir / "readme.txt",
                            subdir,
    ...
    def test_handles_copy_error_gracefully(self, tmp_path: Path):
        ...
        try:
            with patch("shutil.copy2") as mock_copy:
                ...
                config._init_scripts_dir()
                # Should have attempted copies (if any system prompts existed)
        finally:
            _restore_modules(removed)
        ...
        try:
            ...
            with patch.object(sys, "frozen", True, create=True):
                with patch.object(sys, "_MEIPASS", str(tmp_path / "meipass"), create=True):
                    # Create the PyInstaller prompts location
                    meipass_prompts = tmp_path / "meipass" / "aish" / "scripts" / "prompts"
                    meipass_prompts.mkdir(parents=True)
                    (meipass_prompts / "bundled.aish").write_text("bundled script")
...
class TestInitScriptsDirMultipleLocations:
    """Tests for multiple system prompts location handling."""
    ...
    def test_falls_back_to_second_location(self, tmp_path: Path):
        ...
        second_location = tmp_path / "usr" / "local" / "share" / "aish" / "prompts"
